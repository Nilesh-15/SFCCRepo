...
function getGA4ProductObject(product) {
    ...
    obj.item_brand = product.brand ? product.brand : '' ;
    obj.item_variant = product.custom.size ? product.custom.size : '' ;

    if (product.primaryCategory != null) {
        obj.item_category = product.primaryCategory.displayName;
        obj.item_list_name = product.primaryCategory.displayName; // item_list_name GA4 parameter
    } else if (master && master.primaryCategory != null) {
        obj.item_category = master.primaryCategory.displayName;
        obj.item_list_name = master.primaryCategory.displayName; // item_list_name GA4 parameter
    }
...
function getPdpData(res) {
    ...
    if ('product' in res) {
        ...
        obj.ecommerce.detail.products.push(module.exports.getGA4ProductObject(product));
    }
    return obj;
}
...
function getGA4SearchImpressionData(res) {
     ...
     if ('productSearch' in res && 'category' in res.productSearch) {
        ...
        var obj = {
            'event': 'view_item_list',
            'ecommerce': {
                'searchKeyWord':res.productSearch ? res.productSearch.searchKeywords : ''
            }
        };
    }
...
function getGA4OrderProductObject(productLineItem) {
    ...
    obj.quantity = productLineItem.getQuantityValue();
    obj.index = productLineItem.position;
    return obj;
}
...
function getGA4ServicableData(res) {
    var obj = {
        'event': 'not_serviceable_location',
        'ecommerce': {
        }
    };

    if(res.error){
        obj.ecommerce.latitude = res.latitude;
        obj.ecommerce.longitude = res.longitude;
        obj.ecommerce.city = res.city;
        return obj;
    }
...
/** Trigger Registration Login with OTP event*/
function getG4ARegistrationLoginData(res) {
    var queryParams = res.queryString;
    var type = queryParams.split('=')[1];
    var object = {
        "event":"registration_login",
        "ecommerce":{
            "event_type":type == 'false' ? 'login':'registration',
            "firstName":res.account.profile.firstName,
            "lastName":res.account.profile.lastName,
            "email":res.account.profile.email,
            "phoneNumber":res.account.profile.phone
        }
    }
...
/** Trigger Add coupon event */
function getG4AAddCouponLineItem(res){
    var queryParams = res.queryString;
    var coupon = queryParams.split('&')[0];
    var couponCode = coupon.split('=')[1];
    var object = {
        'event':'add_coupon',
        'ecommerce':{
            'coupon':couponCode
...
/** Trigger remove coupon event */
function getG4ARemoveCouponLineItem(res){
    var queryParams = res.queryString;
    var coupon = queryParams.split('&')[0];
    var couponCode = coupon.split('=')[1];
    var object = {
    ...
    if (ga4) {
        ...
        switch (res.action) {
            ...
            case 'Location-Update':
                return module.exports.getGA4ServicableData(res);    
            case 'Account-Show':
                return module.exports.getG4ARegistrationLoginData(res);
            case 'Cart-AddCoupon':
                return module.exports.getG4AAddCouponLineItem(res);
            case 'Cart-RemoveCouponLineItem':
                return module.exports.getG4ARemoveCouponLineItem(res);
            default:
                return false;
        }
    } else {
...
module.exports = {
    ...
    getGA4ServicableData: getGA4ServicableData,
    getG4ARegistrationLoginData:getG4ARegistrationLoginData,
    getG4AAddCouponLineItem:getG4AAddCouponLineItem,
    getG4ARemoveCouponLineItem:getG4ARemoveCouponLineItem
};
