...
var preferences = require('*/cartridge/config/preferences');
var ratings = require('*/cartridge/models/product/decorators/ratings');
...
function getMaxOrderQty(productListItemObject) {
    ...
    if (productListItemObject.product.availabilityModel.inventoryRecord) {
        var perpetual = productListItemObject.product.availabilityModel.inventoryRecord.perpetual;
        var atsValue = productListItemObject.product.availabilityModel.inventoryRecord.ATS.value;
        availableToSell = (DEFAULT_MAX_ORDER_QUANTITY <= atsValue || perpetual) ? 10 : atsValue;
...
function createProductListItemObject(productListItemObject) {
    ...
    if (productListItemObject) {
        ...
        result = {
            ...
            imageObj: new ImageModel(productListItemObject.product, { types: ['medium'], quantity: 'single' }),
